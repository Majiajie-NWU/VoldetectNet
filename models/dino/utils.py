...
def nested_tensors_to_tensor(nested_tensors):
    # 提取每个NestedTensor中的'tensors'属性
    tensors = [nt.tensors for nt in nested_tensors]  # nt 是 NestedTensor 实例

    # 假设每个tensors都有相同的批量大小B和相同的维度C, H, W
    # 将这些tensors沿新的维度（时间点T）堆叠起来
    stacked_tensor = torch.stack(tensors, dim=1)  # 维度变为 [B, T, C, H, W]
