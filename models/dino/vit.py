...
class Transformer_Structure(nn.Module):
    def __init__(self, dim_seq=4096, num_heads=3, dim_head=18, num_encoders=8):
        super().__init__()
        self.cls_token = nn.Parameter(torch.zeros(1, 1, dim_seq))
        self.order_embedding = nn.Parameter(torch.randn(1, 3, dim_seq))
        # self.order_embedding = nn.Parameter(torch.randn(1, 80, dim_seq))
...
class transformer_network(nn.Module):
    ...
    def __init__(self, *, in_channels=1, num_levels=4, f_maps=16, dim_hidden=4096, num_heads=8, dim_head=768,
                 num_encoders=8, num_linear=2, num_class=1):
        super().__init__()
