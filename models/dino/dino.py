...
from .utils import sigmoid_focal_loss, MLP,nested_tensors_to_tensor

from ..registry import MODULE_BUILD_FUNCS
from .dn_components import prepare_for_cdn,dn_post_process
from .MultiHeadTemporalAttentionpy import MultiHeadCrossTemporalAttention
from .FetureFusionpy import FeatureFusion
...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
                    ...
                    att_channels=[512, 1024, 2048],
                 duoqi_chanenels=[2048,2048,2048],
                    ):
        ...
        #ablation
        #self.attention_module=[FeatureFusion(chanel).to(device) for chanel in att_channels]
    ...
    def forward(self, samples: List[NestedTensor], targets:List=None):
        ...
        #-----------------------------v2
        #majiajie add
        #1. muti-sequence feature interation
        all_features = [] #store phase 1-3   "F3 F4 F5"
        all_pos = []
        all_features_fuse = []#store multi-sequence F5
        poss_time=[]

        for sample in samples:#遍历3张多期图片
            features, pos = self.backbone(sample)
            all_features.append(features)
            all_pos.append(pos)
            for l, (feat,p) in enumerate(zip(features,pos)):
                #select F5 feature
                if l==2:
                    all_features_fuse.append(feat)
                    poss_time.append(p)


        #features=all_features[0]#只使用第3期特征图
        #poss=all_pos[0]

        features = all_features[0]  # select  phase 2 of "F3 F4 F5"
        poss = all_pos[0]
        ...
        #先投影再 注意力
        #prepare t=1~3  F5 and masks
        srcs_time = []
        masks_time = []
        for t,feat in enumerate(all_features_fuse):
            src, mask = feat.decompose()
            srcs_time.append(self.input_proj[l](src))
            masks_time.append(mask)
            assert mask is not None
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss,
                                                                            input_query_label, srcs_time,masks_time,poss_time,attn_mask)
        # ----------------------------------------------------------------------------------------------------------------------------------------
