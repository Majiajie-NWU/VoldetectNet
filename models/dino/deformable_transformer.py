...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          num_feature_levels, nhead, enc_n_points,
                                                              ...
                                                              use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type)
            # encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
            #                                               dropout, activation,
        ...
        if num_feature_levels > 1:
            if self.num_encoder_layers > 0:
                self.level_embed_time = nn.Parameter(torch.Tensor(num_feature_levels, d_model))
            else:
                self.level_embed_time = None
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, srcs_time,masks_time,poss_time,attn_mask=None):

        """
        Input:
            - srcs: List of multi-scale features [bs, ci, hi, wi]
            - masks: List of multi-scale masks [bs, hi, wi]
            - refpoint_embed: [bs, num_dn, 4]. None in infer
            - pos_embeds: List of multi-scale pos embeds [bs, ci, hi, wi]
            - tgt: [bs, num_dn, d_model]. None in infer
            #majiajie add
            -srcs_time: List of multi-sequence features [bs, ci, hi, wi]
            -mastks-time: List of multi-sequence masks
            -poss_time:List of multi-sequence pos embeds
            
        """



        assert len(srcs) == len(self.in_channels)
        ...
        #M2F encoder
        #prepare flatten to a long sequence
        if self.num_encoder_layers > 0:
            src_flatten = []
            mask_flatten = []
            ...
            for lvl, (src, mask, pos_embed) in enumerate(zip(srcs_time,masks_time,poss_time)):
                bs, c, h, w = src.shape
                spatial_shape = (h, w)
                ...
                if self.num_feature_levels > 1 and self.level_embed_time is not None:
                    lvl_pos_embed = pos_embed + self.level_embed_time[lvl].view(1, 1, -1)
                else:
            ...
            valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks_time], 1) #2,3,2
            ...
            #enc_ind=self.num_encoder_layers[0]
            #memory   b  t*h*w  256
            #post preocess for selecting t sequence
            t=valid_ratios.shape[1]
            for i,enc_ind in enumerate(self.use_encoder_idx):
                memory=memory.permute(0, 2, 1).reshape(-1, self.hidden_dim, t,h, w).contiguous()  #b,c,t,h,w
                temp=memory.permute(2,0,1,3,4).contiguous()
                proj_feats[enc_ind]=temp[0]
            ...
            for idx in range(len(self.in_channels) - 1, 0, -1):
                feat_high = inner_outs[0]  # F5
                feat_low = proj_feats[idx - 1]  # F4

                feat_high = self.lateral_convs[len(self.in_channels) - 1 - idx](feat_high)

                inner_outs[0] = feat_high
                upsample_feat = F.interpolate(feat_high, scale_factor=2., mode='nearest')  # Upsample
                inner_out = self.fpn_blocks[len(self.in_channels) - 1 - idx](torch.cat([upsample_feat, feat_low], dim=1))
                inner_outs.insert(0, inner_out)  #let calculation result at idx 0
            outs = [inner_outs[0]]
        ...
        lvl_pos_embed_flatten = []
        spatial_shapes = []
        for lvl, (src, mask, pos_embed) in enumerate(zip(outs, masks, pos_embeds)):
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)
        ...
        if self.two_stage_type =='standard':
            ...
            output_memory, output_proposals = gen_encoder_output_proposals(src_flatten, mask_flatten, spatial_shapes,input_hw)
        ...
        # Begin Decoder
        #########################################################
        #重新生成 src_flatten,mask_flatten,lvl_pos_embed_flatten,spatial_shapes ,level_start_index,valid_ratios
