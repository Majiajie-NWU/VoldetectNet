...
#     batch = list(zip(*batch))
#     batch[0] = [nested_tensor_from_tensor_list(images) for images in batch[0]]
#     return tuple(batch)
...
def collate_fn(batch):
    ...
    # 处理包含多个时间点图像的列表
    temporal_images = list(zip(*batch[0]))
    nested_tensors = [nested_tensor_from_tensor_list(images) for images in temporal_images]

    return nested_tensors, batch[1]
...
#     # 假设你的 nested_tensor_from_tensor_list 实现
#     # 将一组 tensor 合并为一个 batch
